...
func semasleep(ns int64) int32 {
	...
	if ns >= 0 {
		...
		if r, err := sem_timedwait((*semt)(unsafe.Pointer(mp.waitsema)), &ts); r != 0 {
			...
			println("sem_timedwait err ", err, " ns ", ns, " id ", mp.id)
			throw("sem_timedwait")
		}
		return 0
	}
...
func newosproc0(stacksize uintptr, fn unsafe.Pointer) {
	...
	for tries := 0; tries < 20; tries++ {
		ret = pthread_create(&tid, &attr, uintptr(fn), nil)
		if ret != _EAGAIN {
			break
		}
...
// miniterrno retrieves a pointer to the thread's errno via
// __errno_location().
func miniterrno() {
	mp := getg().m
	r, _ := syscall0(&libc__errno_location)
	mp.perrno = r
}
...
// tstart_hurd is the pthread start routine, defined in sys_hurd_386.s.
func tstart_hurd(newm *m) uint32
...
func newosproc(mp *m) {
	...
	ret := retryOnEAGAIN(func() int32 {
		return pthread_create(&tid, &attr, abi.FuncPCABI0(tstart_hurd), unsafe.Pointer(mp))
	})
	sigprocmask(_SIG_SETMASK, &oset, nil)
	if ret != 0 {
...
// sigtramp is the signal handler entry point, defined in sys_hurd_386.s.
func sigtramp()
...
func nanotime1() int64 {
	...
	return int64(tp.tv_sec)*1000000000 + int64(tp.tv_nsec)
}
