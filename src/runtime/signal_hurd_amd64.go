...
func (c *sigctxt) rflags() uint64 { return uint64(c.regs().gregs[_REG_RFL]) }
func (c *sigctxt) cs() uint64     { return uint64(c.regs().gregs[_REG_CS]) }

// Hurd's amd64 mcontext does not record the %fs/%gs bases; the Go runtime
// only uses these for dumpregs, so report zero.
